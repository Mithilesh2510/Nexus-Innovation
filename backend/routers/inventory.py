"""
Inventory catalog, medicine details, and demand forecasting API routes. Every route
requires login; branch_id is always resolved through resolve_branch_scope() so a
branch-scoped login can never see another branch's (hospital's) data, regardless of
what it passes as a query param.
"""
...
def medicine_detail(medicine_id: str, branch_id: str = Query(...), user: dict = Depends(get_current_user)):
    effective_branch = resolve_branch_scope(user, branch_id)
    try:
        risk = compute_risk(STORE, medicine_id, effective_branch)
...
def medicine_forecast(
    medicine_id: str,
    branch_id: Optional[str] = Query(None),
    horizon_days: int = Query(21, le=90),
    user: dict = Depends(get_current_user),
):
    effective_branch = resolve_branch_scope(user, branch_id)
    series = STORE.daily_consumption(medicine_id, branch_id=effective_branch)
